...
__all__ = 'RustWasm', 'StdWeb', 'Object', 'Array'
...
class RustWasm:
    def __init__(self, pathname: str, imports: Dict) -> None:
        with open(pathname, 'rb') as f:
            wasm_bytes = f.read()

        def __web_on_grow():
            memory = self.instance.memory
            self.HEAP8 = memory.int8_view()
            self.HEAP16 = memory.int16_view()
            self.HEAP32 = memory.int32_view()
            self.HEAPU8 = memory.uint8_view()
            self.HEAPU16 = memory.uint16_view()
            self.HEAPU32 = memory.uint32_view()

        env = imports['env']
        env.update({ '__web_on_grow': __web_on_grow })

        for name in env.keys():
            env[name] = print_exception(env[name])
        ...
        # LookupError: Function `_RustWasm__web_malloc` does not exist.
        # self.web_malloc = instance.exports.__web_malloc
        # self.web_free = instance.exports.__web_free
...
class StdWeb:
    ...
    def to_py_string(self, pointer: int, length: int) -> str:
        buffer = self.wasm.instance.memory.buffer
        string_bytes = bytes(buffer)[pointer:pointer+length]
